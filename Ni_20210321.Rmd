...
postfix <- ""
    ...
    occ.hist$beta.isco[occ.hist[,"nickel"]==0] <- NA
    ...
    level2.estimates <- summaryBy(as.formula(paste0("beta.isco ~ isco2 + nickel")), data=occ.hist, FUN=function(x){c(mean(x,na.rm=TRUE),length(x[!is.na(x)]))}) 
    ...
    level3.estimates <- summaryBy(as.formula(paste0("beta.isco ~ isco3 + nickel")), data=occ.hist, FUN=function(x){c(mean(x,na.rm=TRUE),length(x[!is.na(x)]))})
...
selected <- occ.hist$nickel==1
occ.hist$log.exposure[selected] <- occ.hist$log.exposure[selected] + fixed.effects[fixed.effects$effect=="nickel" & fixed.effects$nickel==1,"estimate"]
occ.hist$log.exposure.josue <- occ.hist$log.exposure
occ.hist$log.exposure.josue[occ.hist$isco==100000] <- -Inf
    
selected <- !(occ.hist$nickel==0) & occ.hist$year>1970
...
table(occ.hist$nickel,is.finite(occ.hist$log.exposure)) #all lines with nickel should have log.exposure
occ.hist$assignment.level[occ.hist$nickel==0] <- 0
...
save(occ.hist,file=paste0(output.path,"updated.hist.nickel.Rdata")) #file with log.exposure for every job x year with all fixed and random effects
```

```{r select unique jobs}

load(file=paste0(output.path,"updated.hist.nickel.Rdata"))
...
#  tmp <- summaryBy(log.exposure ~ subjctid + year, data=occ.hist, FUN=c(f,length))
  ...
  summary(occ.hist)
  table(occ.hist$nickel,is.na(occ.hist$beta.isco))

  nrow(unique(occ.hist[,c("subjctid","year")])) #1424594 
  sum(occ.hist$selected) #1424594
  table(occ.hist$nickel,occ.hist$assignment.level)
  occ.hist <- occ.hist[occ.hist$selected==1,] #1424594 unique lines
  save(occ.hist,file=paste0(output.path,"unique.hist.nickel.Rdata")) #file with log.exposure for every job x year with all fixed and random effects
##new chunk end
...
result <- data.frame(subjctid=unique(occ.hist$subjctid))
  for (lag in seq(0,20,by=5)) {
    ...
    colnames(tmp)[2] <- paste0("nickel",".last.exposure.lag",lag) #for all selected and exposed and appropriate lag, output max (latest) year exposed
    ...
    colnames(tmp)[2] <- paste0("nickel",".duration.lag",lag) #for all selected, exposed, and appropriate lag, output years of exposure (note that diff exposure elvel treated the same)
    result <- merge(result,tmp,by="subjctid",all.x=TRUE)
    ...
    colnames(tmp)[2] <- paste0("nickel",".cumexposure.lag",lag) #for all selected, exposed, and appropriate lag, sum of total exposure
    result <- merge(result,tmp,by="subjctid",all.x=TRUE)
  }

 table(is.na(result[,paste0("nickel",".last.exposure.lag0")])) #29178 NAs, which are people who are never exposed, hence last exposure = NA 
...
summary(all.subjects$nickel.cumexposure.lag0) #31228   unexposed
summary(all.subjects$nickel.duration.lag0) #31228    unexposed
summary(all.subjects$nickel.last.exposure.lag0) #31228   unexposed

table(is.na(all.subjects$nickel.cumexposure.lag5)) #31251  unexposed
summary(all.subjects$nickel.duration.lag5) #31251  unexposed
summary(all.subjects$nickel.last.exposure.lag5) #31251  unexposed

table(is.na(all.subjects$nickel.cumexposure.lag10)) #31300  unexposed
summary(all.subjects$nickel.duration.lag10) #31300  unexposed
summary(all.subjects$nickel.last.exposure.lag10) #31300 unexposed

table(is.na(all.subjects$nickel.cumexposure.lag15)) #31412  unexposed
summary(all.subjects$nickel.duration.lag15) #31412   unexposed
summary(all.subjects$nickel.last.exposure.lag15) #31412   unexposed

table(is.na(all.subjects$nickel.cumexposure.lag20)) # 31578    unexposed
summary(all.subjects$nickel.duration.lag20) # 31578   unexposed
summary(all.subjects$nickel.last.exposure.lag20) # 31578  unexposed

all.subjects$nickel.cumexposure.lag0[is.na(all.subjects$nickel.cumexposure.lag0)]<-0
all.subjects$nickel.cumexposure.lag5[is.na(all.subjects$nickel.cumexposure.lag5)]<-0
all.subjects$nickel.cumexposure.lag10[is.na(all.subjects$nickel.cumexposure.lag10)]<-0
all.subjects$nickel.cumexposure.lag15[is.na(all.subjects$nickel.cumexposure.lag15)]<-0
all.subjects$nickel.cumexposure.lag20[is.na(all.subjects$nickel.cumexposure.lag20)]<-0

all.subjects$exposureQ4 <- cut(all.subjects$nickel.cumexposure.lag0,breaks=c(-Inf,0,quantile(all.subjects$nickel.cumexposure.lag0[all.subjects$status==0 & all.subjects$nickel.cumexposure.lag0>0],(1:3)/4),Inf))
...
names(all.subjects)
all.subjects.ni<-all.subjects[,c(1,45:84)]
names(all.subjects.ni)

save(all.subjects.ni,file=paste0(output.path,"final.subjects.linked.trimmed.Rdata")) 
...
load(file=paste0(output.path,"final.subjects.linked.Rdata"))
all.subjects2<-read.csv(file=paste0(output.path,"R.syn.nickel copy.csv"),na=".") #OG file from LP
...
plot(all.subjects$nickel.cumexposure.lag0,all.subjects2$nickel_cum0) 
plot(all.subjects$packyrs,all.subjects2$packyrs)#no prob here

summary(all.subjects$nickel.cumexposure.lag0)
summary(all.subjects2$nickel_cum0) #

table(all.subjects$status,all.subjects$exposureQ4)
table(all.subjects2$status,all.subjects2$QQ0) #very similar

#occ.hist exposure
load(file=paste0(output.path,"unique.hist.nickel.Rdata"))
occ.hist2<-occ.hist
...
table(is.finite(occ.hist$log.exposure)) #121856 with exposure in og
table(is.finite(occ.hist2$log.exposure)) #121856 with exposure in new. so no problem with exposure status here. 
